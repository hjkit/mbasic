...
**Note:** This report has been filtered to remove 0 previously reviewed/ignored issues. Original report had 434 total issues.
...
The inconsistency: file_io.py says FileIO.load_file() is used by LOAD command and caller passes result to ProgramManager, but manager.py says ProgramManager.load_from_file() is separate and has its own file I/O. The relationship and usage pattern is unclear.
...
However, the code shows load_file(), save_file(), delete_file(), and file_exists() all raise IOError with messages saying "not yet implemented" or "requires async refactor". This contradicts the earlier statement in the same file that says "Acts as an adapter to backend.sandboxed_fs (SandboxedFileSystemProvider from src/filesystem/sandboxed_fs.py)". If it's an adapter, why can't it delegate these operations like list_files() does?
...
- Total issues in filtered report: 434
- Issues filtered out (already reviewed): 0
- Original total: 434

Filtered on: 2025-11-09 14:21:59
