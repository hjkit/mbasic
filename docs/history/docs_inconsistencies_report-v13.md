...
**Description:** _on_status_click() docstring says it shows 'breakpoint info for ●' but the implementation shows a generic message, not actual breakpoint information

**Affected files:**
- `src/ui/tk_widgets.py`

**Details:**
Docstring says: 'Handle click on status column (show error details for ?, breakpoint info for ●)'
...
**Details:**
feature-reference.md states: "Edit Variable Value (Not implemented) ⚠️ Variable editing is not available in Curses UI. You cannot directly edit values in the variables window."

However, variables.md states: "Variable Editing (Limited)" with "Current Status: ⚠️ Partial Implementation" and lists "What Works" and "What Doesn't Work Yet", implying some editing capability exists.
...
**Description:** UI_FEATURE_COMPARISON.md shows 'Save (interactive)' as unavailable (❌) for CLI, but doesn't clarify that SAVE command works - this contradicts the Notes column explanation
...
The '↑↓' arrows are hardcoded, not using constants from keybindings module. This contradicts the comment's claim that ALL shortcuts use constants.
...
**Details:**
Class docstring says: 'After fixing error, ● becomes visible (automatically handled by set_error() method which checks has_breakpoint flag when clearing errors)'
...
**Details:**
data-types.md states double-precision range as: "±2.938736×10^-308 to ±1.797693×10^308" while cdbl.md states: "range from 2.938735877055719 x 10^-39 to 1.701411834604692 x 10^38". The cdbl.md range appears to be incorrect as it shows 10^-39 to 10^38 which is closer to single-precision range.
...
**Details:**
data-types.md states single-precision range as: "±2.938736×10^-39 to ±1.701412×10^38" while csng.md states: "range from 2.938736 x 10^-39 to 1.701412 x 10^38". The csng.md version omits the ± sign which could be misleading about negative number support.
...
**Details:**
lpos.md uses:
'⚠️ **Not Implemented**: This feature requires line printer hardware...'
...
peek.md uses:
'ℹ️ **Emulated with Random Values**: PEEK does NOT read actual memory...'
...
(Different structure, uses ℹ️ instead of ⚠️, has 'Recommendation' instead of 'Alternative')
...
**Details:**
WAIT doc has brief note: '⚠️ Not Implemented: This feature requires direct hardware I/O port access and is not implemented in this Python-based interpreter.'

WIDTH doc has detailed note: '⚠️ Emulated as No-Op: This statement is parsed for compatibility but performs no operation.' with extensive explanation of what works and what doesn't.
...
**Details:**
feature-reference.md states: "Execution Stack: View the call stack showing: **Access methods:** - Via menu: Ctrl+U → Debug → Execution Stack - Via command: Type `STACK` in immediate mode (same as CLI)"
...
**Details:**
index.md comparison table shows: "Variables Window | Curses: ✓ | CLI: ✗ | Tkinter: ✓ | Web: ✓"
...
web/debugging.md states:
'Use **Run → Stop** or `Ctrl+Q` to stop'
...
**Description:** Inconsistent use of ⚠️ symbol - sometimes means 'partially implemented' and sometimes means 'limited/basic'
...
**Description:** UI_FEATURE_COMPARISON.md shows Find/Replace as ❌ for Web UI but marks it as ⚠️ with 'Tk: implemented, Web: planned' in the Notes
...
The Web column shows ⚠️ but the Notes say 'Web: planned', which should be 📋 according to the legend:
...
**Description:** Feature matrix shows 'Edit variables' as ⚠️ for Curses with 'CLI: immediate mode only' in Notes, but CLI column shows ❌
...
The Notes say 'CLI: immediate mode only' but the CLI column shows ❌ (not available). If CLI can edit variables in immediate mode, it should be ⚠️ with a note, not ❌. This is contradictory.
...
**Description:** The tier label mapping in _search_indexes method has a comment 'Note: UI tier (e.g., 'ui/curses', 'ui/tk') is detected via startswith('ui/') check below and gets '📘 UI' label. Other unrecognized tiers get '📙 Other'.' However, the actual tier_labels dict only defines 'language' and 'mbasic', not showing the UI or Other mappings explicitly.
...
- After fixing error, ● becomes visible (automatically handled by set_error() method which checks has_breakpoint flag when clearing errors)'
...
The PRINT statement uses quotes around "MONTHLY REPORT" but the syntax shown uses backslash quotes which is unusual. Should likely be:
20 PRINT X$; "MONTHLY REPORT"; X$
...
GOTO.md references:
- FOR ••• NEXT (with bullets)
- GOSUB •.. RETURN (with bullets)
- IF ••• THEN[ ••• ELSE] AND IF ••• GOTO (different format)
- ON ••• GOSUB AND ON ••• GOTO (different format)
- WHILE ••• WEND (with bullets)
...
**Details:**
architecture.md states 'Semantic Analyzer: ✅ Complete (18 optimizations)' and 'The semantic analyzer is production-ready' but also says 'Code Generation: ❌ Not implemented (future work)'. Later it says 'Current status: Analysis only (no code generation yet)'. The claim of being 'production-ready' and 'Complete' is misleading when it's only the analysis phase without code generation - it cannot actually produce optimized code, only analyze and report.
...
settings.md states: "Click the ⚙️ Settings icon in the navigation bar" or "Click menu → Settings"
...
The dialog interface diagram shows: "📝 Editor    📊 Limits"
...
- Total issues found: 661
- Code/Comment conflicts: 235
- Other inconsistencies: 426
