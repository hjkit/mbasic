...
class Z88dkCBackend(CodeGenBackend):
    ...
    def generate(self, program: ProgramNode) -> str:
        ...
        code.append('')
        code.append('/* Define heap for malloc */')
        code.append('#pragma output REGISTER_SP = 0xF000')
        code.append('#pragma output CRT_STACK_SIZE = 512')
        code.append('#pragma output CLIB_MALLOC_HEAP_SIZE = 2048')
        code.append('')

        # String system defines and includes
        if self.string_count > 0:
            code.append(f'#define MB25_NUM_STRINGS {self.string_count}')
            code.append('#define MB25_POOL_SIZE 1024  /* 1KB string pool */')
