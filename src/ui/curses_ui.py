...
class ScrollingFiller(urwid.WidgetWrap):
    ...
    def render(self, size, focus=False):
        ...
        cursor_line = text[:edit_pos].count('\n')

        # Get total lines in document
        total_lines = text.count('\n') + 1

        # Calculate target scroll position to center cursor in middle 50% of viewport
        # Target: keep cursor between 25% and 75% of visible area
        target_min_row = int(maxrow * 0.25)
        target_max_row = int(maxrow * 0.75)

        # Calculate top line (first visible line)
        # If we can render the entire document, start at 0
        if total_lines <= maxrow:
            self._top_line = 0
        else:
            cursor_row_in_viewport = cursor_line - self._top_line

            # Scroll up if cursor too high
            if cursor_row_in_viewport < target_min_row:
                self._top_line = max(0, cursor_line - target_min_row)
            # Scroll down if cursor too low
            elif cursor_row_in_viewport > target_max_row:
                self._top_line = min(total_lines - maxrow, cursor_line - target_max_row)

            # Ensure we don't scroll past the end
            if self._top_line + maxrow > total_lines:
                self._top_line = max(0, total_lines - maxrow)

        # Render the edit widget to get full canvas
        canvas = edit.render((maxcol,), focus)
...
class CursesBackend(UIBackend):
    ...
    def _create_ui(self):
        ...
        # Use ScrollingFiller to maintain vim-style cursor centering
        self.editor_filler = ScrollingFiller(self.editor, valign='top')
