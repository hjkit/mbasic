#!/usr/bin/env python3
"""
Documentation and Code Consistency Checker (Version 3)
...
Changes from v2:
- Auto-generates output filename based on highest version in docs/history/
- Uses get_next_report_filename utility to find next available version number
- Outputs to ../../docs/history/docs_inconsistencies_report-v{N}.md
...
Usage:
    python3 utils/checker/check_docs_consistency3.py
...
# Import the filename picker utility
try:
    from get_next_report_filename import get_next_report_filename
except ImportError:
    print("Error: get_next_report_filename module not found")
    print("Make sure get_next_report_filename.py is in the same directory as this script")
    sys.exit(1)
...
def main():
    """Main entry point."""
    print("Enhanced Consistency Checker v3.0")
    print("==================================\n")
    ...
    print("\n** NEW in v3: Auto-generates versioned output filename **\n")
    ...
    try:
        # Get the next available report filename
        print("="*60)
        print("Determining output filename...")
        print("="*60)
        output_filename = get_next_report_filename()
        print()
        ...
        # Save the report
        report_path = analyzer.save_report(all_inconsistencies, code_conflicts, output_filename)
